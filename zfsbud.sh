...
declare -A src_keep_timestamps=()
declare -A src_kept_timestamps=()

declare -A dst_keep_timestamps=()
declare -A dst_kept_timestamps=()
...
set_src_timestamps_to_keep() {
  (( ${#src_keep_timestamps[@]} )) && return 0 # Perform function once.

  for (( i=0; i<=$(config_get src_minutes)-1; i++ )); do ((src_keep_timestamps[$(date +%Y-%m-%d-%H%M -d "-$i minute")]++)); done
  for (( i=0; i<=$(config_get src_hourly)-1; i++ )); do ((src_keep_timestamps[$(date +%Y-%m-%d-%H -d "-$i hour")]++)); done
  for (( i=0; i<=$(config_get src_daily)-1; i++ )); do  ((src_keep_timestamps[$(date +%Y-%m-%d -d "-$i day")]++)); done
  for (( i=0; i<=$(config_get src_weekly)-1; i++ )); do ((src_keep_timestamps[$(date +%Y-%m-%d -d "sunday-$((i + 1)) week")]++)); done
  for (( i=0; i<=$(config_get src_monthly)-1; i++ )); do
    DW=$(($(date +%-W) - $(date -d "$(date -d "$(date +%Y-%m-15) -$i month" +%Y-%m-01)" +%-W)))
    for ((AY = $(date -d "$(date +%Y-%m-15) -$i month" +%Y); AY < $(date +%Y); AY++)); do
      ((DW += $(date -d "$AY"-12-31 +%W)))
    done
    ((src_keep_timestamps[$(date +%Y-%m-%d -d "sunday-$DW weeks")]++))
  done
  for (( i=0; i<=$(config_get src_yearly)-1; i++ )); do
    DW=$(date +%-W)
  ...
  #echo ${!src_keep_timestamps[*]}
  printf "%s\n" "${!src_keep_timestamps[@]}" | sort > src_keep
}

set_dst_timestamps_to_keep() {
  (( ${#dst_keep_timestamps[@]} )) && return 0 # Perform function once.

  for (( i=0; i<=$(config_get dst_minutes)-1; i++ )); do ((dst_keep_timestamps[$(date +%Y-%m-%d-%H%M -d "-$i minute")]++)); done
  for (( i=0; i<=$(config_get dst_hourly)-1; i++ )); do ((dst_keep_timestamps[$(date +%Y-%m-%d-%H -d "-$i hour")]++)); done
  for (( i=0; i<=$(config_get dst_daily)-1; i++ )); do  ((dst_keep_timestamps[$(date +%Y-%m-%d -d "-$i day")]++)); done
  for (( i=0; i<=$(config_get dst_weekly)-1; i++ )); do ((dst_keep_timestamps[$(date +%Y-%m-%d -d "sunday-$((i + 1)) week")]++)); done
  for (( i=0; i<=$(config_get dst_monthly)-1; i++ )); do
    DW=$(($(date +%-W) - $(date -d "$(date -d "$(date +%Y-%m-15) -$i month" +%Y-%m-01)" +%-W)))
    for ((AY = $(date -d "$(date +%Y-%m-15) -$i month" +%Y); AY < $(date +%Y); AY++)); do
      ((DW += $(date -d "$AY"-12-31 +%W)))
    done
    ((dst_keep_timestamps[$(date +%Y-%m-%d -d "sunday-$DW weeks")]++))
  done
  for (( i=0; i<=$(config_get dst_yearly)-1; i++ )); do
    DW=$(date +%-W)
    for ((AY = $(($(date +%Y) - i)); AY < $(date +%Y); AY++)); do
      ((DW += $(date -d "$AY"-12-31 +%W)))
    done
  ...
  #echo ${!dst_keep_timestamps[*]}
  printf "%s\n" "${!dst_keep_timestamps[@]}" | sort > dst_keep
}

src_keep_snapshot?() {
  for ts in "${!src_keep_timestamps[@]}"; do
    if [[ $1 == "$ts"* ]]; then
      if ((${src_kept_timestamps[$ts]})); then
...
dst_keep_snapshot?() {
  for ts in "${!dst_keep_timestamps[@]}"; do
    if [[ $1 == "$ts"* ]]; then
      if ((${dst_kept_timestamps[$ts]})); then
        # already kept one snapshot matching this timestamp, so don't keep any others,
        # however, this snapshot might match other desired timestamps, so keep going
        continue
      fi
      let dst_kept_timestamps[$ts]++
      return 0
    fi
  done
  return 1
...
src_rotate_snapshots() {
  ...
  for i in "${!source_snapshots[@]}"; do
    ...
    && ! src_keep_snapshot? $snapshot_timestamp \
...
dst_rotate_snapshots() {
  set_dst_timestamps_to_keep
  dst_kept_timestamps=()
  for i in "${!destination_snapshots[@]}"; do
    # Remove all snapshots prefixed accordingly and not matching the
    # keep_timestamps pattern; always keep the most recent snapshot and the last
    # common snapshot.
    snapshot_timestamp=${source_snapshots[i]#*"@$snapshot_prefix"}
    if [[ "${destination_snapshots[i]}" == *"@$snapshot_prefix"* ]] \
    && ! dst_keep_snapshot? $snapshot_timestamp \
    && [[ "${destination_snapshots[i]}" != "${destination_snapshots[-1]}" ]] ; then
      if [ ! -v last_snapshot_common ] \
      || [[ "${destination_snapshots[i]}" != "$dataset@$last_snapshot_common" ]] ; then
        msg "Deleting destination snapshot: ${destination_snapshots[i]}"
        [ ! -v dry_run ] && $remote_shell zfs destroy $recursive_destroy -f "${destination_snapshots[i]}"
        unset "destination_snapshots[i]"
      fi
    fi
  done
...
send_resume() {
  ...
  if [ ! -v dry_run ]; then
    if [ -v remote_shell ]; then
      ! zfs send -w $verbose -t "$resume_token" | mbuffer -q -r "$RATE" -m "$BUF" | zstd | $remote_shell "zstd -d | zfs recv $resume -F -d -u $pool" && return 1
    else
...
process_dataset() {
  ...
  [ -v remove_old ] && src_rotate_snapshots
  ...
  [ -v remove_old ] && dst_rotate_snapshots
}
