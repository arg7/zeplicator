...
- **`zpipe.c`**: High-performance C utility for pipe monitoring (replaces Perl/PV).
- **`Makefile`**: Assembly script that bundles libraries, the orchestrator, and the `zpipe` binary into a single file.
- **`build/zep`**: The production-ready bundle generated by `make` (renamed from zpl).
- **`build/zpipe`**: The compiled C binary for IO monitoring.
