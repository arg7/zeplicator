    ...
    if [[ "$TRANSFER_DONE" == true ]]; then
        ...
        if [[ "$PROMOTE" == false && "$IS_DONOR" == false && "$DRY_RUN" == false ]]; then
            ...
            if [[ $SSH_STATUS -eq 2 ]]; then
                ...
                if [[ "$REPL_POLICY" == "resilience" ]]; then
                    echo "${CHAIN_PREFIX}  ⚠️  WARNING: Split-Brain detected downstream from $hop_node. Continuing due to policy=resilience..."
                    # We continue the outer loop to the next hop_node
                    continue
                else
